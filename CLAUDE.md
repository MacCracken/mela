# Mela — Claude Code Instructions

## Project Identity

**Mela** (Hindi: मेला — fair/marketplace) — App marketplace for AGNOS
...
ark (marketplace packages), daimon (agent discovery). Mela is the centralized marketplace for AGNOS applications and AI agents, providing discovery, distribution, and trust verification.
...
- All packages must be Ed25519-signed — reject unsigned artifacts
- SHA-256 integrity verification on every download
- Marketplace API calls use rustls only — no OpenSSL
...
- Do not accept unsigned packages — all marketplace artifacts must be signed
- Do not use OpenSSL — rustls only for TLS
